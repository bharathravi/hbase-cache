...
public final class ProtobufUtil {
  ...
  public static Get toGet(
    ...
    if (proto.hasCustomId()) {
      get.setCustomId(proto.getCustomId());
    }
    return get;
  }
  ...
  public static ClientProtos.Get toGet(
    ...
    builder.setCustomId(get.getCustomId());
    return builder.build();
  }
...
// Start helpers for Client
  ...
  public static Result getRowOrBefore(final ClientProtocol client,
    ...
    try {
      GetResponse response = client.get(null, request);
      if (!response.hasResult()) {System.out.println("Null!");return null;}
      return toResult(response.getResult());
    } catch (ServiceException se) {
      throw getRemoteException(se);
    }
  ...
  /**
   * TODO(bharath): Clone of above to allow custom if tagging
   * @param client
   * @param regionName
   * @param row
   * @param family
  ...
  public static Result getRowOrBefore(final ClientProtocol client,
        final byte[] regionName, final byte[] row,
        final byte[] family, int customId) throws IOException {
      GetRequest request =
        RequestConverter.buildGetRowOrBeforeRequest(
          regionName, row, family, customId);
      try {
        GetResponse response = client.get(null, request);
        if (!response.hasResult()) {System.out.println("Null!");return null;}
        return toResult(response.getResult());
