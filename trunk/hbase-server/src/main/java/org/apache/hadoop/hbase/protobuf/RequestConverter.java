...
/**
  ...
  /**
   * TODO(bharath): Clone of above, to allow custom id tagging
   * @param regionName
   * @param row
   * @param family
   * @return
   */
  public static GetRequest buildGetRowOrBeforeRequest(
        final byte[] regionName, final byte[] row, final byte[] family, int customId) {
      GetRequest.Builder builder = GetRequest.newBuilder();
      RegionSpecifier region = buildRegionSpecifier(
        RegionSpecifierType.REGION_NAME, regionName);
      builder.setClosestRowBefore(true);
      ...
      getBuilder.setRow(ByteString.copyFrom(row));
      getBuilder.addColumn(columnBuilder.build());
      getBuilder.setCustomId(customId);
      builder.setGet(getBuilder.build());
      return builder.build();
    }
