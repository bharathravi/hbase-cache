...
public class StoreFileScanner implements KeyValueScanner {
  ...
  public static final Map<Path, AtomicLong> seekCounts = new HashMap<Path, AtomicLong>();
  ...
  public static List<StoreFileScanner> getScannersForStoreFiles(
      ...
      boolean cacheBlocks,
      boolean usePread, int customId) throws IOException {
    return getScannersForStoreFiles(files, cacheBlocks,
                                   usePread, false, customId);
  }
  ...
  public static List<StoreFileScanner> getScannersForStoreFiles(
      ...
      boolean isCompaction, int customId) throws IOException {
  ...
  public static List<StoreFileScanner> getScannersForStoreFiles(
      Collection<StoreFile> files, boolean cacheBlocks, boolean usePread,
      boolean isCompaction, ScanQueryMatcher matcher, int customId) throws IOException {
  ...
  public boolean seek(KeyValue key) throws IOException {
    ...
    // TODO(adding counters for HFile)
    Path thisFile = reader.getHFileReader().getPath();

    if (seekCounts.containsKey(thisFile)) {
      seekCounts.get(thisFile).incrementAndGet();
    } else {
      seekCounts.put(thisFile, new AtomicLong(1));
    }
